...
DMC_am_SPC_monomer.py
CS446 -- DMC Research
Spring 2024
----------------------------------------------------------------------------------------------
Enter `DMC_am_SPC_monomer.py` into the terminal to run DMC on the system of the water monomer 
with the SPC/Fw model of water. 
    ...
    def simulation(self):
        ...
        while self.trial_iter < self.nTrials:
            ...
            # if a division error occurs due to poor intialization conditions...
                ...
                # if the randomizations were seeded, terminate, as otherwise we would need to ignore the user's seed
                if self.seed is not None:
                    exit('Stochastic processes associated with this seed lead to a division by zero. Exiting.')

                # otherwise, print a message to let the user know a new trial is being started, and reinitialize and
                # run the trial
                else:
...
def position(count):
    ...
    # randomly initialize the particles of the system
    pos = np.random.normal(0, 0.5, (count, 1, 3, 3))
