...
DMC_am_SPC_dimer_3.py
CS446 -- DMC Research
Spring 2024
----------------------------------------------------------------------------------------------
Enter `DMC_am_SPC_dimer_3.py` into the terminal to run DMC on the system of the water dimer
with the SPC/Fw model of water, and the third specified set of parameters. 
...
# equilibrium H-O bond length in bohr radii
equilibriumLength = 1.0/0.529177

# equilibrium HOH angle in radians
equilibriumAngle = 112.0*np.pi/180.0

# spring constant for the length of H-O bond in hartrees/(bohr radii)^2
kl = 1059.162*(0.529)**2*(4.184/2625.5)

# spring constant for the HOH bond angle in hartrees/radians^2
...
# -- data of TIP4 model, for use in initializing positions --
...
std_H2O = np.zeros((3, 4))

# set the homogeneous coordinate
std_H2O[:, 3] = 1

# set the first hydrogen particle
std_H2O[0, :3] = np.array([rOH, 0, 0])

# set the second hydrogen particle
std_H2O[1] = rotation_matrix('z', aHOH) @ std_H2O[0]
std_H2O[1, :3] = rOH*(std_H2O[1, :3]/np.linalg.norm(std_H2O[1, :3]))
...
def coulombic(dmc):
    ...
    for srcmlclidx in range(walkers.nMolecules-1):
        ...
        # obtain arrays of indices so that every combination of H-to-O is obtained
        ...
        # obtain all intermolecular distances between hydrogen and oxygen atoms, and reshape 
        # to nicely list each per walker
        ...
        # sum all the coulombic terms between hydrogen and oxygen atoms within each walker, and 
        # add to total coulombic
        col += np.sum(qH*qO/dHO, axis=1)

        # obtain arrays of indices so that every combination of O-to-H is obtained
        ...
        # obtain all intermolecular distances between oxygen and hydrogen atoms, and reshape
        # to nicely list each per walker
        ...
        # sum all the coulombic terms between oxygen and hydrogen atoms within each walker, and
        # add to total coulombic
        col += np.sum(qO*qH/dOH, axis=1)

        # obtain arrays of indices so that every combination of O-to_O is obtained
        ...
        # obtain all intermolecular distances between oxygen atoms, and reshape 
        # to nicely list each per walker
        ...
        # sum all the coulombic terms between oxygen atoms within each walker, and add to
        # total coulombic
...
def intra_potential(dmc):
    '''
    Computes the part of the potential energy of each walker due to intramolecular forces.

    dmc: DMC object. Simulation for which to compute the potential energy due to intramolecular forces.

    return: ndarray. shape=(nWalkers). Potential energy due to intramolecular forces per walker.
    '''
    # obtain the data structure containing the walkers of the simulation
    walkers = dmc.walkers

    # initialize an array to store the potential energy due to intramolecular forces per walker
    intraEnergy = np.zeros(walkers.nWalkers)

    # iterate through each water molecule of the system
    for watermlcl in range(1, walkers.molecules2counts['H2O']+1):

        # obtain the H-O bonds
        bd1 = walkers.vecs[walkers.molecule(f'H2O {watermlcl}')][:, 0, walkers.particle('H2O O 1'), walkers.particle('H2O H 1')]
        bd2 = walkers.vecs[walkers.molecule(f'H2O {watermlcl}')][:, 0, walkers.particle('H2O O 1'), walkers.particle('H2O H 2')]

        # obtain the lengths of the H-O bonds
        len1 = walkers.dists[walkers.molecule(f'H2O {watermlcl}')][:, 0, walkers.particle('H2O O 1'), walkers.particle('H2O H 1')]
        len2 = walkers.dists[walkers.molecule(f'H2O {watermlcl}')][:, 0, walkers.particle('H2O O 1'), walkers.particle('H2O H 2')]

        # compute the potential energy due to the H-O bond stretches
        potLen1 = 1/2*kl*(len1-equilibriumLength)**2
        potLen2 = 1/2*kl*(len2-equilibriumLength)**2

        # obtain the HOH bond angle
        angles = bond_angle(bd1, bd2, len1, len2)

        # compute the potential energy due to the HOH bond angle
        potAngle = 1/2*ka*(angles-equilibriumAngle)**2

        # for the first water molecule...
        if watermlcl == 1:
            # store the mean H-O bond lengths and HOH bond angle
            dmc.add_data('Mean HO Bond Length 1', np.mean(len1))
            dmc.add_data('Mean HO Bond Length 2', np.mean(len2))
            dmc.add_data('Mean HO Bond Angle', np.mean(angles))

        # compute the intramolecular potential due to this water molecule as the sum of the potential 
        # due to its H-O bond stretches and HOH bond angle
        intraEnergy += potLen1 + potLen2 + potAngle

    # return the potential energy due to intramolecular forces
    return intraEnergy
...
def energy(dmc):
    ...
    return intra_potential(dmc) + inter_potential(dmc)
...
def diffusion(dmc):
    ...
    # obtain the shape of all H particles in the system
    H_shape = arr[:, walkers.mlclidx['H2O'], walkers.ptclidx['H2O H'], :].shape

    # obtain the shape of all O particles in the system
    O_shape = arr[:, walkers.mlclidx['H2O'], walkers.ptclidx['H2O O'], :].shape

    # diffuse the H particles of the system
    arr[:, walkers.mlclidx['H2O'], walkers.ptclidx['H2O H']] += np.random.normal(0.0, np.sqrt(timeStep/walkers.atomic_mass('H')), H_shape)

    # diffuse the O particles of the system
...
def position(count):
    ...
    # THIS IS DONE SO THAT THE WALKERS IN SPC/FW AND TIP4 ARE INITIALIZED IN THE SAME WAY

    # randomly initialize the position- and rotation-vectors of a rigid representation of the system
    rigid_init = np.zeros((count, 2, 6))
    rigid_init[:, 0] = np.random.normal(0, 0.5, (count, 6))
    rigid_init[:, 1] = np.random.normal(10, 0.5, (count, 6))

    # convert to the flexible representation of the system
    pos = rigid_arr_to_std_arr(rigid_init, std_H2O)
    return pos
...
if __name__ == '__main__':
    ...
    walkers = Walkers(particles2mass, molecules2counts, 'flexible', std_H2O) 
    ...
    # register special data to save over the course of the simulation
    sim.register_data('Mean HO Bond Length 1')
    sim.register_data('Mean HO Bond Length 2')
    sim.register_data('Mean HO Bond Angle')

    # save all data to disk
    sim.save(sim.walker_populations)
    sim.save(sim.reference_energies)
    sim.save('Mean HO Bond Length 1')
    sim.save('Mean HO Bond Length 2')
    sim.save('Mean HO Bond Angle')
    ...
    # HO Bond Length 1 over last trial after convergence
    plt.hist(sim.data['Mean HO Bond Length 1'][sim.convergence_point:], 100)
    plt.title(f'Distribution of HO Bond Length 1 Over Last Trial, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    plt.xlabel('Length (Bohr Radii)')
    plt.ylabel('Frequency')
    plt.axvline(equilibriumLength, color='red', label=f'Equilibrium Length ({equilibriumLength:.3f} Bohr Radii)')
    plt.legend()
    plt.show()

    # HO Bond Length 2 over last trial after convergence
    plt.hist(sim.data['Mean HO Bond Length 2'][sim.convergence_point:], 100)
    plt.title(f'Distribution of HO Bond Length 2 Over Last Trial, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    plt.xlabel('Length (Bohr Radii)')
    plt.ylabel('Frequency')
    plt.axvline(equilibriumLength, color='red', label=f'Equilibrium Length ({equilibriumLength:.3f} Bohr Radii)')
    plt.legend()
    ...
    # obtain a flattened ndarray containing all post-convergence first H-O bond lengths
    all_ho_lengths1 = sim.extract_post_convergence_flat(sim.load('Mean HO Bond Length 1'))

    # HO Bond Length 1 over all trials
    plt.hist(all_ho_lengths1, 100)
    plt.title(f'Distribution of HO Bond Length 1 Over All Trials, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    plt.xlabel('Length (Bohr Radii)')
    plt.ylabel('Frequency')
    plt.axvline(equilibriumLength, color='red', label=f'Equilibrium Length ({equilibriumLength:.3f} Bohr Radii)')
    plt.legend()
    ...
    # obtain a flattened ndarray containing all post-convergence second H-O bond lengths
    all_ho_lengths2 = sim.extract_post_convergence_flat(sim.load('Mean HO Bond Length 2'))

    # HO Bond Length 2 over all trials
    plt.hist(all_ho_lengths2, 100)
    plt.title(f'Distribution of HO Bond Length 2 Over All Trials, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    plt.xlabel('Length (Bohr Radii)')
    plt.ylabel('Frequency')
    plt.axvline(equilibriumLength, color='red', label=f'Equilibrium Length ({equilibriumLength:.3f} Bohr Radii)')
    plt.legend()
    ...
    # HO Bond Angle over last trial after convergence
    plt.hist(sim.data['Mean HO Bond Angle'][sim.convergence_point:], 100)
    plt.title(f'Distribution of HO Bond Angles Over Last Trial, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    plt.xlabel('Radians')
    plt.ylabel('Frequency')
    plt.axvline(equilibriumAngle, color='red', label=f'Equilibrium Angle ({equilibriumAngle:.3f} Radians)')
    plt.legend()
    plt.show()

    # obtain a flattened ndarray containing all post-convergence HOH bond angles
    all_angles = sim.extract_post_convergence_flat(sim.load('Mean HO Bond Angle'))

    # HO Bond Angle over all trials
    plt.hist(all_angles, 100)
    plt.title(f'Distribution of HO Bond Angle Over All Trials, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    plt.xlabel('Radians')
    plt.ylabel('Frequency')
    plt.axvline(equilibriumAngle, color='red', label=f'Equilibrium Angle ({equilibriumAngle:.3f} Radians)')
    plt.legend()
