...
class Walkers:
    ...
    def get_vector(self, vecs, sourcemlcl, destmlcl, sourceptcl, destptcl):
        return vecs[self.molecule(sourcemlcl)][:, self.molecule(destmlcl) - self.molecule(sourcemlcl), self.particle(sourceptcl), self.particle(destptcl), :]
...
class dmc:
    ...
    def mean_ref_val(self):
        print(f'variance is {np.var(self.ref_vals)}')
...
def vectors(walkers, mlcl):
    '''
    bonds = (nWalkers, nMolecules, nParticles (source), nParticles (destination), 3)
    '''
    arr = walkers.to_arr()

    mlcl_info = mlcl.split()
    mlcl_name = mlcl_info[0]
    mlcl_idx = int(mlcl_info[1])-1

    mlclidx = walkers.mlclidx[mlcl_name][mlcl_idx]

    ptcl_vectors = np.zeros((walkers.nWalkers, walkers.nMolecules, walkers.nParticles, walkers.nParticles, 3))

    for ptclidx in range(walkers.molecules2particlecounts[mlcl_name]):
        ptcl_vectors[:, :, ptclidx, :, :] = arr[:, mlclidx:, :, :] - arr[:, mlclidx, ptclidx, :].reshape(walkers.nWalkers, 1, 1, 3)

    return ptcl_vectors

def all_vectors(walkers):
    '''
    bonds = (nWalkers, nMolecules, nParticles (source), nParticles (destination), 3)
    '''
    ...
    for mlclidx in range(walkers.nMolecules):

        mlcl_name = walkers.index2molecule[mlclidx].split()[0]

        ptcl_vectors.append(np.zeros((walkers.nWalkers, walkers.nMolecules - mlclidx, walkers.nParticles, walkers.nParticles, 3)))

        for ptclidx in range(walkers.molecules2particlecounts[mlcl_name]):
            ptcl_vectors[mlclidx][:, :, ptclidx, :, :] = arr[:, mlclidx:, :, :] - arr[:, mlclidx, ptclidx, :].reshape(walkers.nWalkers, 1, 1, 3)
...
def potentialEnergyFunction(walkers):
    ...
    bond = bond_vectors(walkers, 'CO', 'C', 'O')
    distances = np.linalg.norm(bond, axis=2)

    return np.sum(((1/2)*1.2216*(distances-0.59707)**2), axis=1)
...
def diffusionFunction(walkers):
    ...
    Cshape = arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO C'], :].shape
    Oshape = arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO O'], :].shape
    arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO C'], :] += np.random.normal(0.0, np.sqrt(timeStep/walkers.atomic_mass('C')), Cshape)
    arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO O'], :] += np.random.normal(0.0, np.sqrt(timeStep/walkers.atomic_mass('O')), Oshape)
...
def posFunction(count):
    ...
    '''
    pos = np.random.rand(count, 1, 2, 3)
    return pos
