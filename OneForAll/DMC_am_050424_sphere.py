...
class DMC:
    ...
    def adjust_walker_population(self):        
        ...
        prob_delete = np.exp(-(greater_vals-self.ref_val)*self.time_step, dtype=np.float128)
        ...
        prob_replicate = np.exp(-(lesser_vals-self.ref_val)*self.time_step, dtype=np.float128) - 1.0
...
if __name__ == '__main__':
    ...
    sim = DMC()
    sim.setParams(timeStep, nTimeSteps, nWalkers, potentialEnergyFunction, diffusionFunction, posFunction, walkers, nTrials)
    sim.run()
