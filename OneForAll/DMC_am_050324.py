...
and enter `python3 DMC_am_050324.py` into a terminal to run a DMC simulation with the specified 
...
class Walkers:
    def __init__(self, particles2mass, molecules2counts, init_cap=10000):
        ...
        # dictionary. string -> int. maps '<bond-name>' to an index within ndarray `arr`.
        # self.bdidx = dict()
        ...
        #     ref_dict[mlcl] = 0
        #     self.molecules2bonds[mlcl] = []
        
        # # setup the bdidx
        # for bond in bonds:
        #     mlcl = bond.split()[0]
        #     self.bdidx[bond] = ref_dict[mlcl]

        #     ref_dict[mlcl] += 1

        #     self.molecules2bonds[mlcl].append(bond)
    ...
    # def get_bonds(self):
    #     '''returns: ndarray. shape=(nWalkers, nBonds)'''
    #     bond_vecs = [] 

    #     for mlclidx in range(self.nMolecules):
    #         mlcl = self.index2molecule[mlclidx]
    #         bond_vecs.append(np.zeros(self.nWalkers, len(self.molecules2bonds[mlcl]), 3))
            
    #         for bd in self.molecules2bonds[mlcl]:
    #             info = bd.split()

    #             atom1 = info[1] + ' ' + info[2]
    #             atom2 = info[3] + ' ' + info[4]

    #             bond_vecs[mlclidx] = self.get_vector(mlcl + ' ' + mlclidx+1, mlcl, atom1, )
            
    #     for bdidx in range(self.nBonds): 
    #         bond = self.bonds[bdidx]
    #         mlcl = bond[0]

    #         for mlclidx in range(self.molecules2counts[mlcl]):
    #             # bond_vecs[:, bdidx] = pass
    #             pass

    #         bond_vecs[:, bdidx] = np.array(self.vecs[[self.molecule(mlcl + ' ' + str(mlclidx)) for mlclidx in range(self.molecules2counts[mlcl])]]) \
    #             [:, :, self.molecule(mlcl + ' ')]         
...
def posFunction(count):
    ...
    '''
    pos = np.random.rand(count, 1, 3, 3)
    return pos
