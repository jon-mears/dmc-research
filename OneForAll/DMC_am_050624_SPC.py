...
class DMC:
    ...
    def setParams(self, time_step, nTimeSteps, nWalkers, val_func, diffuse_func, gen_func, walkers, nTrials=1, seed=None):
        ...
        self.data[self.reference_energies] = np.zeros(self.nTimeSteps)
        self.data[self.walker_populations] = np.zeros(self.nTimeSteps)
    ...
    def compute_ref_val(self):
        return np.mean(self.val_func(self)) + self.penalty()
    ...
    def convergence_point(self, data, granularity=2, var_threshold=1e-5):
        if granularity == np.infty:
            for step in range(int(self.nTimeSteps/2), self.nTimeSteps):
                if np.var(self.data[data][step:]) < var_threshold:
                    return step
            
        for step in range(int(self.nTimeSteps/2), self.nTimeSteps, int(self.nTimeSteps/granularity)):
            if np.var(self.data[data][step:]) < var_threshold:
                return step
        
        print(f'{data} does not appear to converge in trial #{self.trial_iter}. returning first step of simulation.')
        return 0
    ...
    def run(self):
        path = os.path.dirname(os.path.abspath(__file__)) + '/sim_data'
        ...
        while self.trial_iter < self.nTrials:
            try:
                ...
                self.cpoint = self.convergence_point(self.reference_energies)
                self.convergence_points[self.trial_iter] = self.cpoint
                
                for entry in self.data:
                    self.mean_data[entry][self.trial_iter] = np.mean(self.data[entry][self.cpoint:])
                ...
                if self.seed is not None:
                    exit('stochastic processes associated with this seed lead to a division by zero.')
                else:
                    print('stochastic processes led to division by zero. now starting a new simulation.')
                    self.reset()
                    self.run()
    
    def mean_over_trials(self, data):
        return np.mean(self.mean_data[data])
    ...
    def load(self, name):
        ...
        if self.save_data[name]:
            for trial_iter in range(self.nTrials):
                d[trial_iter] = np.genfromtxt(f'{path}/sim_data/{name}_{trial_iter}', delimiter=',')[1:, 1:].reshape(1000)
...
def rigid_matrices(arr):
    # arr = walkers.to_arr()
    
    translations = np.zeros((arr.shape[0], arr.shape[1], 4, 4))
    translations[:, :, :, :] = np.eye(4)

    translations[:, :, :3, 3] = arr[:, :, :3]

    xrotations = np.zeros((arr.shape[0], arr.shape[1], 4, 4))
    xrad = arr[:, :, 3]
    xrotations[:, :, :, :] = np.eye(4)
    ...
    yrotations = np.zeros((arr.shape[0], arr.shape[1], 4, 4))
    yrad = arr[:, :, 4]
    yrotations[:, :, :, :] = np.eye(4)
    ...
    zrotations = np.zeros((arr.shape[0], arr.shape[1], 4, 4))
    zrad = arr[:, :, 5]
    zrotations[:, :, :, :] = np.eye(4)
...
def rigid_arr(arr, std_mlcl):
    matrices = rigid_matrices(arr)
...
origin_H2OM[0, :3] = np.array([equilibriumAngle, 0, 0])

# second hydrogen particle
origin_H2OM[1] = rotation_matrix('z', equilibriumAngle) @ origin_H2OM[0]
origin_H2OM[1, :3] = equilibriumLength*(origin_H2OM[1, :3]/np.linalg.norm(origin_H2OM[1, :3]))
...
'''
def potentialEnergyFunction(dmc):
...
def posFunction(count):
    ...
    # mlcls = np.zeros((count, 2, 6))

    # mlcl1 = np.random.rand(count, 1, 6) - 1
    # mlcl2 = np.random.rand(count, 1, 6) + 1

    # mlcls[:, 0] = mlcl1.reshape(count, 6)
    # mlcls[:, 1] = mlcl2.reshape(count, 6)

    pos[:, 0] = np.random.normal(0, 0.5, (count, 3, 3))
    pos[:, 1] = np.random.normal(10, 0.5, (count, 3, 3))
...
if __name__ == '__main__':
    ...
    print(f'simulation converges approximately to {sim.mean_over_trials(sim.reference_energies)}')

    ### PLOTTING ###
    
    # walker population over last trial after convergence
    plt.hist(sim.data[sim.walker_populations][sim.cpoint:])
    plt.title('Distribution of Walker Populations Over a Single Trial')
    plt.xlabel('Population')
    plt.ylabel('Frequency')
    plt.show()

    # walker population over all trials
    all_walker_populations = sim.load(sim.walker_populations).flatten()
    plt.hist(all_walker_populations)
    plt.title(f'Distribution of Walker Populations Over {nTrials} Trials')
    plt.xlabel('Population')
    plt.ylabel('Frequency')
    plt.show()
    ...
    plt.title('Reference Energies Over a Single Trial')
    plt.xlabel('Time Step')
    plt.ylabel('Reference Energy (Hartrees)')
    plt.axvline(sim.cpoint, color='red', label='Post-Equilibration')
    plt.legend()
    plt.show()

    # reference energies past equilibration of last trial
    plt.plot(np.arange(sim.cpoint, sim.nTimeSteps), sim.data[sim.reference_energies][sim.cpoint:])
    plt.title('Reference Energies Past Equilibration')
    plt.xlabel('Time Step')
    plt.ylabel('Reference Energy (Hartrees)')
    plt.axhline(sim.mean_data[sim.reference_energies][sim.nTrials-1], color='red', label='Mean Reference Energy')
    plt.legend()
    plt.show()