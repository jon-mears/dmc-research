...
class DMC:
    ...
    def adjust_walker_population(self):        
        ...
        if self.walkers.nWalkers == 0:
            a = self.walkers.dists[self.walkers.molecule('H2O 1')][:, self.walkers.molecule('H2O 2')]
            DF = pd.DataFrame(a.reshape(a.shape[0], int(a.size/a.shape[0])))
            DF.to_csv('error_dists.csv')
...
def posFunction(count):
    ...
    HPos = np.random.rand(count, 2, 2, 3) 

    OPos1 = np.random.rand(count, 1, 1, 3)

    OPos2 = np.random.rand(count, 1, 1, 3)

    displacement_vec = OPos2 - OPos1
    unit_displacement_vec = displacement_vec/np.linalg.norm(displacement_vec)

    OPos2 = OPos1 + np.linalg.norm(displacement_vec)*unit_displacement_vec

    pos[:, :, :2, :] = HPos
    pos[:, 0, 2, :] = OPos1.reshape(count, 3)
    pos[:, 1, 2, :] = OPos2.reshape(count, 3)
