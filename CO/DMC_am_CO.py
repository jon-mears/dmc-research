...
Spring 2024
----------------------------------------------------------------------------------------------
Enter `DMC_am_CO.py` into the terminal to run DMC on the system of the carbon monoxide bond
...
particles2mass = {'C': 12, 'O': 15.99491461957}

# dictionary: string -> int. Maps molecule names to their number of occurrences in the relevant
# molecular system.
molecules2counts = {'CO': 1}

# equilibrium C-O bond length in bohr radii
equilibriumLength = 0.59707

# spring constant for the length of C-O bond in hartrees/(bohr radii)^2
kl = 1.2216
...
def intra_potential(dmc):
    ...
    # obtain the length of the C-O bond per walker
    len1 = walkers.dists[walkers.molecule(f'CO 1')][:, 0, walkers.particle('CO C 1'), walkers.particle('CO O 1')]

    dmc.add_data('Mean Bond Length', np.mean(len1))

    # obtain the potential energy due to the bond length
    potLen = 1/2*kl*(len1-equilibriumLength)**2
...
def diffusion(dmc):
    ...
    # obtain the shape of all C particles in the system
    C_shape = arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO C'], :].shape

    # obtain the shape of all O particles in the system
    O_shape = arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO O'], :].shape

    # diffuse the C particles of the system
    arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO C']] += np.random.normal(0.0, np.sqrt(timeStep/walkers.atomic_mass('C')), C_shape)

    # diffuse the O particles of the system
    arr[:, walkers.mlclidx['CO'], walkers.ptclidx['CO O']] += np.random.normal(0.0, np.sqrt(timeStep/walkers.atomic_mass('O')), O_shape)
...
def position(count):
    ...
    pos =  np.random.normal(0.0, 1, (count, 1, 2, 3))
...
if __name__ == '__main__':
    ...
    walkers = Walkers(particles2mass, molecules2counts, 'flexible') 
    ...
    print(f'Simulation Converges Approximately to {np.mean(all_reference_energies)} Hartrees. Expected: {4.94317e-3} Hartrees.\n')
    ...
    # CO bond length over last trial post-convergence
    plt.hist(sim.data['Mean Bond Length'][sim.convergence_point:], 100)
    plt.title(f'Distribution of CO Bond Length Over Last Trial, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    ...
    # obtain a flattened ndarray containing all post-convergence CO bond lengths
    all_co_lengths = sim.extract_post_convergence_flat(sim.load('Mean Bond Length'))

    # CO bond length over all trials
    plt.hist(all_co_lengths, 100)
    plt.title(f'Distribution of CO Bond Length Over All Trials, Post-Convergence ({os.path.basename(__file__).split(".")[0]})')
    plt.xlabel('Length (Bohr Radii)')
    plt.ylabel('Frequency')
    plt.axvline(equilibriumLength, color='red', label=f'Equilibrium Length ({equilibriumLength:.3f} Bohr Radii)')
    plt.legend()
    ...
    plt.axhline(4.94317e-3, color='yellow', label=f'Known Zero-Point Energy: {4.94317e-3:.3f} Hartrees')
