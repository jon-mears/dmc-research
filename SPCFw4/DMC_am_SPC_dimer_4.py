...
DMC_am_SPC_dimer_4.py
CS446 -- DMC Research
Spring 2024
----------------------------------------------------------------------------------------------
Enter `DMC_am_SPC_dimer_4.py` into the terminal to run DMC on the system of the water dimer
with the SPC/Fw model of water, and the fourth specified set of parameters. 
...
def position(count):
    ...
    '''
    # randomly initialize the particles of the system, with the water molecules separated
    pos = np.zeros((count, 2, 3, 3)) 
    pos[:, 0] = np.random.normal(0, 0.5, pos[:, 0].shape)
    pos[:, 1] = np.random.normal(3, 0.5, pos[:, 1].shape)
